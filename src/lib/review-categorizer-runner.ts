...
} from "./review-categorizer";

/**
 * Sample reviews extracted from seed.sql
 * Includes all 50 BurgerHub reviews with their metadata
 */
const REVIEWS_DATA = [
 ...
 */
export async function runReviewCategorization(): Promise<void> {
  console.log("🍔 Starting Review Categorization Analysis...\n");
  ...
  for (const [categoryName, stats] of sortedCategories) {
    ...
    console.log("-".repeat(80));
    console.log(`Total Reviews: ${stats.count}`);
    console.log(`Percentage: ${stats.percentage.toFixed(1)}%`);
    console.log(`Average Rating: ${stats.avg_rating.toFixed(2)}/5`);
    console.log(`Sentiment Breakdown:`);
    console.log(`  - Positive: ${stats.sentiment_breakdown.positive}`);
    console.log(`  - Negative: ${stats.sentiment_breakdown.negative}`);
    console.log(`  - Neutral: ${stats.sentiment_breakdown.neutral}`);
  }
