  ...
  return (
    <div className="p-6 rounded-3xl bg-[#0b0f19] border border-white/5 shadow-xl space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold text-white tracking-tight">Ask Your Reviews</h3>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {QUICK_PROMPTS.map((prompt, idx) => (
            <button
              ...
              disabled={isLoading}
              className="px-3 py-1.5 rounded-xl bg-[#06080e] border border-white/5 hover:border-indigo-500/50 hover:bg-gray-900/50 text-xs font-medium text-gray-300 hover:text-white transition-all disabled:opacity-50 text-left"
            >
              {prompt}
            </button>
          ))}
      ...
      <form
        ...
        className="flex items-center gap-2 p-1.5 rounded-2xl bg-[#06080e] border border-white/5 focus-within:border-indigo-500 transition-colors"
      >
        <input
          type="text"
          value={question}
      ...
      {/* Loading Skeleton */}
      {isLoading && (
        <div className="p-5 rounded-2xl bg-[#0b0f19] border border-white/5 animate-pulse space-y-3">
          <div className="h-4 w-1/3 bg-gray-800 rounded"></div>
          <div className="h-3 w-full bg-gray-800/80 rounded"></div>
      ...
      {activeAnswer && !isLoading && (
        <div className="p-5 rounded-3xl bg-[#06080e] border border-white/5 shadow-lg shadow-indigo-950/20 space-y-4 animate-in fade-in duration-200">
          {/* Question Tag */}
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
            ...
            <span className="text-[10px] text-gray-500 font-medium">Data-Grounded Response</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
            ...
            {activeAnswer.supporting_statistics && activeAnswer.supporting_statistics.length > 0 && (
              <div className="p-3.5 rounded-2xl bg-[#0b0f19] border border-white/5 space-y-2">
          ...
          {activeAnswer.recommended_action && (
            <div className="p-3.5 rounded-2xl bg-indigo-950/30 border border-indigo-500/30 space-y-1">
              <span className="text-[10px] uppercase font-bold text-indigo-300 tracking-wider flex items-center gap-1">
                <Lightbulb className="w-3 h-3 text-indigo-400" /> Recommended Action
              </span>
          ...
          {activeAnswer.referenced_issues && activeAnswer.referenced_issues.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap pt-1">
              ...
              <span className="text-[10px] text-gray-500">Referenced Categories:</span>
              {activeAnswer.referenced_issues.map((iss, i) => (
                <span
                  key={i}
                  className="text-[10px] font-semibold px-2 py-0.5 rounded bg-gray-900/50 border border-white/5 text-gray-300"
                >
                  {iss}
                </span>
              ))}
          ...
          <div className="space-y-1.5">
            {history.slice(1, 4).map((h, i) => (
              <button
                ...
                className="w-full p-2.5 rounded-xl bg-[#06080e] border border-white/5 hover:border-white/10 text-left flex items-center justify-between text-xs text-gray-300 hover:text-white transition-colors"
              >
                <span className="truncate">&ldquo;{h.question}&rdquo;</span>
                <div className="flex items-center gap-1 text-[10px] text-gray-500 shrink-0 ml-2">
