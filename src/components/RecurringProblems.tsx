...
export function RecurringProblems({ issues, onSelectReview, isLoading }: RecurringProblemsProps) {
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="p-5 rounded-2xl bg-[#0b0f19] border border-white/5 animate-pulse space-y-3">
            <div className="h-4 w-3/4 bg-gray-800 rounded"></div>
            <div className="h-3 w-1/2 bg-gray-800/80 rounded"></div>
  ...
  return (
    <div className="space-y-6">
      {/* Header Info */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-[#0b0f19] border border-white/5">
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-rose-500/10 text-rose-400 border border-rose-500/20">
            <AlertOctagon className="w-5 h-5 text-rose-400" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-white tracking-tight">Recurring Problems Intelligence Engine</h3>
        ...
        <div className="flex items-center gap-2 text-xs text-gray-400 self-start sm:self-auto">
          <span className="px-2.5 py-1 rounded-lg bg-gray-950 border border-gray-800 font-semibold text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {issues.map((issue) => {
          ...
          const severityCardStyle = isCritical
            ? 'border-rose-500/30 bg-[#0b0f19] relative overflow-hidden'
            : isHigh
            ? 'border-amber-500/20 bg-[#0b0f19] hover:border-amber-500/40'
            : 'border-white/5 bg-[#0b0f19] hover:border-white/10'
          ...
          const severityBadge =
            issue.severity_level === 'Critical' ? (
              <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-md bg-rose-500/20 text-rose-300 border border-rose-500/40">
                <Flame className="w-3 h-3 text-rose-400 animate-pulse" /> Critical ({issue.severity_score}/10)
              </span>
              ...
              <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-500/20 text-amber-300 border border-amber-500/30">
                <AlertCircle className="w-3 h-3 text-amber-400" /> High ({issue.severity_score}/10)
              </span>
            ) : issue.severity_level === 'Medium' ? (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-yellow-500/10 text-yellow-300 border border-yellow-500/20">
          ...
          const trendBadge =
            issue.trend_direction === 'increasing' ? (
              ...
              <span className="flex items-center gap-1 text-xs font-medium text-gray-400 bg-gray-950 px-2 py-0.5 rounded-md border border-gray-800">
                <Minus className="w-3.5 h-3.5" /> Stable
              </span>
            ) : (
              <span className="text-[10px] text-gray-500 italic bg-gray-950 px-2 py-0.5 rounded border border-gray-800">
          ...
          return (
            <div
              ...
              className={`p-5 rounded-2xl border transition-all duration-200 cursor-pointer flex flex-col justify-between space-y-4 group ${severityCardStyle}`}
            >
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[10px] font-semibold uppercase px-2 py-0.5 rounded bg-gray-950 border border-gray-800 text-gray-400">
                    {issue.category}
                  </span>
                  {severityBadge}
                </div>
              ...
              {/* Mentions & Trend Row */}
              <div className="p-3 rounded-xl bg-gray-900/50 border border-white/5 flex items-center justify-between z-10">
                <div>
                  <span className="text-lg font-extrabold text-white block leading-none">
                    {issue.mention_count}{' '}
                    <span className="text-xs font-normal text-gray-400">mentions</span>
                  </span>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs border-t border-white/5 pt-2.5 z-10">
                <div className="flex items-center gap-1.5 text-gray-400">
                  <Star className="w-3.5 h-3.5 text-amber-400" />
                  <span>
                    Avg: <strong className="text-white">{issue.average_rating}★</strong>
                  </span>
                </div>
                ...
                <div className="flex items-center gap-1.5 text-gray-400 justify-end">
                  ...
                  <span>
                    Urgency: <strong className="text-amber-300">{issue.average_urgency}/10</strong>
                  </span>
                </div>
              </div>
              ...
              {issue.sub_issues_breakdown.length > 0 && (
                <div className="space-y-1">
                  <span className="text-[10px] uppercase font-semibold text-gray-500 block">Top Patterns</span>
                  <div className="flex items-center gap-1 flex-wrap">
                    {issue.sub_issues_breakdown.slice(0, 2).map((sub, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 rounded bg-gray-900/50 border border-white/5 text-[10px] text-gray-300 truncate max-w-[200px]"
                      >
                        {sub.name} ({sub.count})
                      </span>
                    ))}
              ...
              <div className="flex items-center justify-between text-xs font-semibold text-indigo-400 group-hover:text-indigo-300 pt-1">
      ...
      {selectedIssue && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-3xl bg-[#06080e] border border-white/5 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            {/* Modal Header */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-bold text-white">{selectedIssue.issue_type}</h3>
                    <span className="text-[10px] font-semibold uppercase px-2 py-0.5 rounded bg-gray-950 border border-gray-800 text-gray-400">
              ...
              <div className="space-y-3">
                {selectedIssue.example_reviews.map((rev) => {
                  ...
                  return (
                    <div
                      key={rev.id}
                      className="p-4 rounded-2xl bg-[#0b0f19] border border-white/5 space-y-2.5 hover:border-white/10 transition-colors"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-xs text-white">{rev.customer_name}</span>
                      ...
                      {rev.ai_response && (
                        <div className="p-2.5 rounded-xl bg-indigo-950/20 border border-indigo-500/10 text-[11px] text-indigo-200/90 flex items-start gap-2">
