...
export function ReviewCard({
  ...
  const priorityConfig = isCritical
    ? {
        label: 'CRITICAL',
        badge: 'bg-rose-500/10 text-rose-700 dark:bg-rose-500/20 dark:text-rose-300 border-rose-300 dark:border-rose-500/40 shadow-sm',
        card: 'border-rose-200 dark:border-rose-500/30 bg-rose-50/40 dark:bg-[#0b0f19] relative overflow-hidden',
        icon: <Flame className="w-3.5 h-3.5 text-rose-500 dark:text-rose-400 animate-pulse" />,
      }
    : isHigh
    ? {
        label: 'HIGH',
        badge: 'bg-amber-500/10 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300 border-amber-300 dark:border-amber-500/30',
        card: 'border-amber-200 dark:border-amber-500/20 bg-amber-50/30 dark:bg-[#0b0f19] hover:border-amber-300 dark:hover:border-amber-500/40',
        icon: <AlertCircle className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" />,
      }
    : isMedium
    ? {
        label: 'MEDIUM',
        badge: 'bg-yellow-500/10 text-yellow-700 dark:bg-yellow-500/10 dark:text-yellow-300 border-yellow-300 dark:border-yellow-500/20',
        card: 'border-slate-200 dark:border-white/5 bg-white dark:bg-[#0b0f19] hover:border-slate-300 dark:hover:border-white/10 shadow-sm',
        icon: <AlertCircle className="w-3.5 h-3.5 text-yellow-500 dark:text-yellow-400" />,
      }
    : {
        label: 'LOW',
        badge: 'bg-emerald-500/10 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300 border-emerald-300 dark:border-emerald-500/20',
        card: 'border-slate-200 dark:border-white/5 bg-white dark:bg-[#0b0f19] hover:border-slate-300 dark:hover:border-white/10 shadow-sm',
        icon: <span className="w-2 h-2 rounded-full bg-emerald-500 dark:bg-emerald-400" />,
      }
  ...
  const sentimentColor =
    review.analysis?.sentiment === 'positive'
      ...
      ? 'text-rose-700 dark:text-rose-400 bg-rose-500/10 border-rose-300 dark:border-rose-500/20'
      : 'text-slate-700 dark:text-gray-400 bg-slate-500/10 border-slate-300 dark:border-gray-500/20'

  // Status Badge
  const statusBadge =
    review.response_status === 'sent' ? (
      <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/20">
        <CheckCircle2 className="w-3 h-3" /> Sent
      </span>
    ) : review.response_status === 'approved' ? (
      <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-300 dark:border-blue-500/20">
        <CheckCircle2 className="w-3 h-3" /> Approved
      </span>
    ) : (
      <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-300 dark:border-amber-500/20">
  ...
  return (
    <div
      className={`p-5 rounded-2xl border transition-all duration-200 flex flex-col justify-between space-y-4 relative overflow-hidden group shadow-sm ${priorityConfig.card}`}
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-bold text-sm text-slate-900 dark:text-white">{review.customer_name}</span>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center text-amber-400 text-xs">
              ...
              <span className="ml-1 text-[11px] text-slate-500 dark:text-gray-400 font-normal">({review.rating}/5)</span>
            </div>
      ...
      {/* Review Text */}
      <p className="text-xs text-slate-700 dark:text-gray-200 line-clamp-3 leading-relaxed font-normal italic">
      ...
      {hasAnalysis ? (
        <div className="flex items-center gap-2 flex-wrap text-[11px] pt-1">
          ...
          {(review.analysis?.primary_issue || review.analysis?.issue) && (
            <span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-gray-900/50 border border-slate-200 dark:border-white/5 text-slate-700 dark:text-gray-300">
              Issue: <strong className="text-slate-900 dark:text-white capitalize">{review.analysis.primary_issue || review.analysis.issue}</strong>
            </span>
          )}

          {/* Urgency Score */}
          {review.analysis?.urgency_score !== undefined ? (
            <span className="px-2 py-0.5 rounded-md bg-amber-50 dark:bg-gray-900/50 border border-amber-200 dark:border-white/5 text-amber-700 dark:text-amber-300">
              Urgency: <strong>{review.analysis.urgency_score}/10</strong>
            </span>
          ) : review.analysis?.urgency ? (
            <span className="px-2 py-0.5 rounded-md bg-amber-50 dark:bg-gray-900/50 border border-amber-200 dark:border-white/5 text-amber-700 dark:text-amber-300 capitalize">
          ...
          {review.analysis?.emotion && (
            <span className="px-2 py-0.5 rounded-md bg-purple-50 dark:bg-gray-900/50 border border-purple-200 dark:border-white/5 text-purple-700 dark:text-purple-300 capitalize">
              {review.analysis.emotion}
            </span>
          )}
        </div>
        ...
        <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-gray-900/50 border border-slate-200 dark:border-white/5 text-xs text-slate-600 dark:text-gray-400 flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 disabled:opacity-50"
          >
      ...
      {hasResponse && (
        <div className="p-3 rounded-xl bg-indigo-50/70 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-500/10 space-y-1.5 relative">
          ...
          <p className="text-[11px] text-slate-800 dark:text-indigo-200/90 line-clamp-2 leading-relaxed font-normal italic">
      ...
      <div className="border-t border-slate-200 dark:border-gray-800/80 pt-3 flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-1.5 flex-wrap">
          {/* Analyze / Re-Analyze */}
          <button
            onClick={() => onAnalyze(review.id)}
            disabled={isAnalyzing}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-gray-900/50 border border-slate-200 dark:border-white/5 hover:bg-slate-200 dark:hover:bg-gray-800 text-xs font-medium text-slate-700 dark:text-gray-300 transition-colors disabled:opacity-50"
            title={hasAnalysis ? 'Re-run Gemini analysis' : 'Analyze review'}
          >
            {isAnalyzing ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-600 dark:text-indigo-400" />
            ) : hasAnalysis ? (
              <RotateCw className="w-3.5 h-3.5 text-slate-500 dark:text-gray-400" />
            ) : (
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg border text-xs font-medium transition-all ${
              showQuickAssist
                ? 'bg-indigo-600/10 dark:bg-indigo-600/30 border-indigo-500 text-indigo-700 dark:text-indigo-200'
                : 'bg-slate-100 dark:bg-gray-900/50 border-indigo-200 dark:border-indigo-500/20 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-gray-800 hover:border-indigo-400 dark:hover:border-indigo-500/40'
            }`}
          ...
          {!hasResponse ? (
            ...
            <button
              onClick={() => onGenerateResponse(review.id)}
              disabled={isGenerating}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-gray-900/50 border border-slate-200 dark:border-white/5 hover:bg-slate-200 dark:hover:bg-gray-800 text-xs font-medium text-indigo-700 dark:text-indigo-300 transition-colors disabled:opacity-50"
          ...
          {hasResponse && (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-gray-900/50 border border-slate-200 dark:border-white/5 hover:bg-slate-200 dark:hover:bg-gray-800 text-xs font-medium text-slate-700 dark:text-gray-300 transition-colors"
          ...
          {hasResponse && isPending && (
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-xs font-medium text-white shadow-sm transition-colors disabled:opacity-50"
        ...
        <button
          onClick={() => onOpenDetails(review)}
          className="flex items-center gap-1 text-xs font-semibold text-slate-700 dark:text-gray-300 hover:text-slate-950 dark:hover:text-white group-hover:translate-x-0.5 transition-all"
        >
